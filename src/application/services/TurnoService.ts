...
export class TurnoService {
  ...
  async tomarTurno(
    ...
    try {
      ...
      const turnosConflictivos = await this.turnoRepository.findByGuia(
        guiaInfo.id,
        {
      ...
      const hayConflicto = turnosConflictivos.some((turno) => {
        return (
          (turnoExistente.fechaInicio >= turno.fechaInicio &&
            turnoExistente.fechaInicio < turno.fechaFin) ||
          (turnoExistente.fechaFin > turno.fechaInicio &&
            turnoExistente.fechaFin <= turno.fechaFin) ||
          (turnoExistente.fechaInicio <= turno.fechaInicio &&
            turnoExistente.fechaFin >= turno.fechaFin)
        );
      });
  ...
  async usarTurno(
    ...
    try {
      ...
      const turno = await this.turnoRepository.usarTurno(
        turnoId,
        observaciones
      );
  ...
  async liberarTurno(
    ...
    try {
      ...
      if (
        ![EstadoTurno.TOMADO, EstadoTurno.EN_USO].includes(
  ...
  async terminarTurno(
    ...
    try {
      ...
      if (!horasCalculadas) {
        const duracionMs =
          turnoExistente.fechaFin.getTime() -
          turnoExistente.fechaInicio.getTime();
        horasCalculadas =
          Math.round((duracionMs / (1000 * 60 * 60)) * 100) / 100; // Redondear a 2 decimales
      }

      const turno = await this.turnoRepository.terminarTurno(
        turnoId,
        horasCalculadas
      );
